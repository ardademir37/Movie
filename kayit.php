...
<head>
    ...
    <title>FilmFlix - Keyfini Çıkar</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        ...
        .user-name { font-weight: bold; font-size: 13px; color: #fff; }
        ...
        .dropdown-content { display: none; position: absolute; top: 100%; left: 0; background: #111; min-width: 180px; border-top: 2px solid #f5c518; max-height: 50vh; overflow-y: auto; box-shadow: 0 8px 16px rgba(0,0,0,0.5); }
        ...
        .hero-title { font-size: clamp(30px, 6vw, 60px); font-weight: 800; margin-bottom: 10px; color: #f5c518; text-transform: uppercase; text-shadow: 2px 2px 15px black; line-height: 1.1; }
        .hero-desc { font-size: clamp(14px, 2vw, 18px); margin-bottom: 25px; max-width: 550px; text-shadow: 1px 1px 5px black; color: #ddd; line-height: 1.4; }
        ...
        .slider-container { display: flex; gap: 10px; overflow-x: auto; scroll-behavior: smooth; scrollbar-width: none; -ms-overflow-style: none; padding: 10px 0; }
        ...
        .movie-card:hover { transform: scale(1.08); z-index: 10; box-shadow: 0 10px 20px rgba(0,0,0,0.6); }
        ...
        /* GRID RESPONSIVE */
        .grid-container { display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 20px; padding: 0 5%; margin-top: 20px; }
        .page-header { padding: 80px 5% 20px 5%; border-bottom: 1px solid #333; }
        
        .add-list-btn { position: absolute; top: 10px; right: 10px; background: rgba(0,0,0,0.7); color: white; border: 1px solid #fff; width: 30px; height: 30px; border-radius: 50%; display: flex; justify-content: center; align-items: center; font-size: 12px; transition: 0.3s; }
        .movie-card:hover .add-list-btn { background: #f5c518; color: #000; border-color: #f5c518; }

        /* MOBİL ÖZEL AYARLAR */
        @media (max-width: 768px) {
            header { padding: 10px 15px; }
            .nav-links { display: none; } /* Mobilde ana linkleri gizle (İstersen burger eklenebilir) */
            .user-name { display: none; }
            .hero { height: 50vh; padding: 0 20px; }
            .scroll-btn { display: none; } /* Mobilde swipe yapıldığı için oklara gerek yok */
            .movie-card { width: 140px; }
            .movie-card img { height: 200px; }
            .grid-container { grid-template-columns: repeat(2, 1fr); gap: 10px; }
            .category-row { padding: 0 15px; }
...
<body>
    ...
    <header>
        <div class="nav-left">
            ...
            <ul class="nav-links">
                ...
                <li class="dropdown">
                    <a href="#">Kategoriler <i class="fa-solid fa-caret-down"></i></a>
        ...
        <div class="nav-right">
            <form action="index.php" method="GET" class="search-box">
                <input type="text" name="arama" placeholder="Ara..." value="<?php echo $_GET['arama'] ?? ''; ?>">
    ...
    <?php if ($mod == 'anasayfa'): ?>
        ...
        <div class="hero">
            ...
            <div style="display: flex;">
                ...
                <a href="index.php?liste_islem=1&fid=<?php echo $heroFilm['id']; ?>" class="btn-hero" style="background:rgba(255,255,255,0.2); color:white; backdrop-filter:blur(5px);">
                    ...
                    <div class="slider-container" id="<?php echo $bolum['id']; ?>">
                        <?php foreach($bolum['liste'] as $f): ?>
                            <div class="movie-card" onclick="location.href='film-detay.php?id=<?php echo $f['id']; ?>'">
                                ...
                                <a href="index.php?liste_islem=1&fid=<?php echo $f['id']; ?>" class="add-list-btn" onclick="event.stopPropagation();"><i class="fa-solid fa-plus"></i></a>
                                <?php if($bolum['id'] == 's5'): ?>
                                    <div style="position:absolute; bottom:0; width:100%; height:3px; background:#f5c518;"></div>
                                <?php endif; ?>
                            </div>
                        <?php endforeach; ?>
                    </div>
    ...
    <?php else: ?>
        ...
        <?php else: ?>
            <div style="text-align:center; padding:100px 20px; color:#555;">
                <i class="fa-solid fa-magnifying-glass fa-3x" style="margin-bottom:20px;"></i><br>
                Maalesef aradığın içeriği bulamadık.
            </div>
        <?php endif; ?>
        
        <div style="text-align:center; margin-top:40px;">
            <a href="index.php" style="border: 1px solid #444; color:white; padding:10px 25px; border-radius:30px; font-size:14px;">Geri Dön</a>
        </div>
    ...
    <script>
        ...
        // AJAX Kütüphane İşlemi
        document.addEventListener('DOMContentLoaded', function() {
            let buttons = document.querySelectorAll('.add-list-btn');
            buttons.forEach(function(btn) {
                btn.addEventListener('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    
                    let urlParams = new URLSearchParams(this.getAttribute('href').split('?')[1]);
                    let filmId = urlParams.get('fid');
